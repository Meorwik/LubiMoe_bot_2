...
from aiogram.utils.markdown import strikethrough
...
class InlineCalendar:
    ...
    def init(self,
        ...
        if month_names is None:
            month_names = ['Янв', 'Фев', 'Maр', 'Апр', 'Maй', 'Июн', 'Июл', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек']
    ...
    def get_keyboard(self, chat_id: Optional[int] = None):
        ...
        for i in range(1, mrange[1] + 1):  # adding active days
            ...
            if curr_date < user_info.min_date:
                ...
                if is_reserved(curr_date):
                    rows[-1].append(
                        types.InlineKeyboardButton(text=str(i), callback_data=InlineCalendar.BASE_CALLBACK.new(
                            action=Actions.PICK_DAY.name, data='0'
                    )))
                else:
